...
def jishuangongshi(x, y):
    ...
    hulitianshu = int(input('请输入住院护理天数（非生活护理）：\n'))
    tinggongliuxinqiyueshu = float(input('请输入停工留薪期月数（数字）：\n'))
    ...
    if (x+1) > 4:
        ...
        z = z - 1
        shenghuohulifeidengjifeilv = [0.6, 0.5, 0.4, 0.3]
        temp1 = float('%.2f' % (yuepinggongzhi * shenghuohulifeidengjifeilv[z]))
        print('\n\n根据以上条件计算，工伤待遇为：\n')
        print('生活护理费（每月）:', temp1, '元', '（生活护理费以上年职工平均工资为计算基数）\n')
