...
class WIN_DIR_DEL(object):
    ...
    CMD_dir = 'dir %s:\*%s /a/s/p >c:\\users\\lenovo\\desktop\\文件查找结果-%s.txt'  # WIN命令行的文件查找命令，>号后面是桌面路径，用于存放查找结果
    CMD_del = 'del %s:\*%s /a/s/p'                                                  # WIN命令行的文件删除命令
    All_PanFu = ['c', 'd', 'e', 'f', 'g', 'h', 'i']                                 # WIN系统常见硬盘盘符

    # 以下函数为文件查找命令
    def WIN_dir(self):
        PanFu = input('\n请问要在哪个或哪些硬盘中查找文件（比如：c  或cde）\n如果是所有硬盘，请输入：s\n> ')
        WenJianMin = input('\n请输入要查找的文件名（比如：通知）\n如果针对某一类文件（比如：ppt pdf），请输入文件名后缀（比如：.ppt .pdf）\n> ')
        if 's' in PanFu:
